...
class eventually_Theme_Plugin_Enhancements {
	...
	static function init() {
		...
		if ( ! $instance ) {
			$instance = new eventually_Theme_Plugin_Enhancements;
		}
	...
	function __construct() {
		...
		$this->plugins = array(			
			array(
				...
				'name'    => 'Font Awesome 5 Menus',
				'download' => 'https://github.com/cogdog/font-awesome-5-menus',
				'message' => sprintf(
					esc_html__( 'The %1$s allows you to create a menu of social icon links.', 'eventually' ),
					'<strong>' . esc_html__( 'Font Awesome 5 for Menus plugin', 'eventually' ) . '</strong>' ),
			),
		);
...
*/
	...
	function admin_notices() {
		...
		if ( get_user_meta( get_current_user_id(), 'eventually_admin_notice', true ) === 'dismissed' ) {
			return;
		}
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' To do this, activate %1$s. %2$s', 'eventually' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate Now', 'eventually' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				if ($install_url) {
					...
					$notice .= sprintf(
						esc_html__( ' To do this, install %1$s. %2$s', 'eventually' ),
						esc_html( $plugin['name'] ),
						( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install Now', 'eventually' ) . '</a>' : ''
					);
				} elseif ( $plugin['download']  ) {
					$notice .= sprintf(
						esc_html__( ' To use this plugin, download %1$s. %2$s and manually install.', 'eventually' ),
						esc_html( $plugin['name'] ),
						( $plugin['download'] ) ? '<a href="' . $plugin['download'] . '" target="_blank">' . esc_html__( 'Download source', 'eventually' ) . '</a>' : ''
					);	
				}
			}
...
add_action( 'admin_head', array( 'eventually_Theme_Plugin_Enhancements', 'init' ) );

function eventually_enqueue_scripts() {
	// Add the admin JS if the notice has not been dismissed
	if ( is_admin() && get_user_meta( get_current_user_id(), 'eventually_admin_notice', true ) !== 'dismissed' ) {

		// Adds our JS file to the queue that WordPress will load
		wp_enqueue_script( 'eventually_admin_script', get_template_directory_uri() . '/includes/splot-plugins.js', array( 'jquery' ), '20190801', true );

		// Make some data available to our JS file
		wp_localize_script( 'eventually_admin_script', 'eventually_admin', array(
			'eventually_admin_nonce' => wp_create_nonce( 'eventually_admin_nonce' ),
		));
	}
}
add_action( 'admin_enqueue_scripts', 'eventually_enqueue_scripts' );
 ...
 */
function eventually_dismiss_admin_notice() {
	// Verify the security nonce and die if it fails
	if ( ! isset( $_POST['eventually_admin_nonce'] ) || ! wp_verify_nonce( $_POST['eventually_admin_nonce'], 'eventually_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'eventually' ) );
	}
	// Store the user's dimissal so that the notice doesn't show again
	update_user_meta( get_current_user_id(), 'eventually_admin_notice', 'dismissed' );
	// Send success message
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'eventually' )
	) );
}
add_action( 'wp_ajax_eventually_admin_notice', 'eventually_dismiss_admin_notice' );
